...
This example walks through the computation of principal components (PCA) of genomic
variant data of 2,504 people from the 1000 genomes project[^1].
The example projects all of the variant
data for one chromosome into a three-dimensional subspace, and then plots
the result. I think the example is popular perhaps because it's very effective at clustering people by ethnicity. It's often used to illustrate "big data" analysis in genomics, even though the data are not particularly big. The point of this example is not to say that PCA on genomic variants is profound, but rather that it's _easy_.
...
library(threejs)
load("https://raw.githubusercontent.com/bwlewis/1000_genomes_examples/master/chr20_pca.RData")
...
pop = read.table("ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/20131219.populations.tsv",sep="\t",header=TRUE)
super = pop[,3]
names(super) = pop[,2]
