...
a common problem in computational genomics and other problems. At least
two R packages can very rapidly perform this computation: the IRanges
package by Pages, Aboyoun and Lawrence in the bioconductor[^1].
and the recent and widely popular data.table package by Dowle, Short, Lianoglou, Srinivasan, and others[^2].
...
hardware. R faster and better-suited to solving problems like the one described
here.
...
2. Count the number of variants that overlap each gene by gene sybol
...
f = function(file)
{
  ...
  out = sprintf("%s.RData",file)
  p = pipe( cmd, open="r")
  name = gsub(".phase.*","",file)
  print(name)
  x = read.table(p, stringsAsFactors=FALSE)
...
commas (the remainder of the file is tab separated). However, R handles the
...
functionality but, unfortunately, with a different syntax). Note the trick of
ordering the first data.frame column, which leads to a dramatic file I/O
performance increase in this example.
...
may lead to over committing of some CPU resources, but can still improve
performance.
...
takes only seconds) and don't bother to save it:
```{r, eval=FALSE}
...
The populate R data.table package includes a fast overlap join function
...
join portion runs in about 0.8 to 1 seconds per chromosome,
slightly faster than the `foverlaps` function available in the data.table package.
...
I couldn't figure out an easy way to
process the doubly-delimited variant data format either within Vertica or from
shell scripts, so I use R to parse the variant data (see above) and save it to
a tab separated file that Vertica can read. The lack of easy data munging
capability is a serious drawback in my opinion of most database systems.
```
...
what the cause of this difference is.
