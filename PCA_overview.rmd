...
pairs along our DNA. Sequences of _base pairs_ define all our genomic traits
like eye color and whether we have freckles. Despite the immense length of our
genome, there is remarkably little variation from one person to the next.
...
Importantly, the likelihood of certain diseases may be encoded in our genomic
sequences.  The study of modified DNA from cells degraded by cancer is also an
importantly and heavily studied topic.

Scientists analyze genomic variants using a huge and constantly expanding
...
We arranged variant data from the 1000 Genomes Project ins a very sparse matrix
...
Our approach breaks the problem up into sequences of smaller problems.  Solving
the problem by breaking it up into manageable pieces has the advantage of
promoting scalability. Those pieces can be run on other CPU cores or even
...
In fact, the above plot colors the data by "Superpopulation" and that corresponds
very well to the displayed data clusters.
```{r,eval=TRUE,echo=FALSE,}
library(DT)
datatable(data.frame(Color=c("red", "yellow", "green", "blue", "purple"),
Superpopulation=c("AFR", "AMR", "EAS", "EUR", "SAS")), options = list(dom='t', columnDefs=list(list(targets=0:1, class="dt-right"))), rownames=FALSE)
```
