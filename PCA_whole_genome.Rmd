...
The same code with mc.cores=16 finishes in about 69 minutes on the Amzon
r3.8xlarge AMI test machine.
...
28 matrix vector products and took about 3.5 hours on my quad core home PC, or
only about 1.7 times faster than the cross product method despite using only
about one fifth as many flops.  As mentioned in the IRLBA notes above, the
...
The same problem finishes in about 19 minutes on the r3.8xlarge Amazon test
sytstem, or nearly 4x faster on that machine. The better performance on the
Amazon system is due to the large amount of RAM on that machine. The whole
problem can fit in RAM and the data files remain in a fast buffer cache.
